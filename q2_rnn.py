...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if n_pad>0:
            sent=sent[:]+[zero_vector]*n_pad
            lab=lab[:]+[zero_label]*n_pad
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        h = tf.zeros((tf.shape(x)[0], self.config.hidden_size), name = "h")
        #h=tf.constant(tf.zeros((None,Config.hidden_size)))
        ### END YOUR CODE
        ...
        preds = tf.stack(preds,axis = 0)  # shape: T * batch_size * n_classes
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits = mask_preds, labels = mask_label)
